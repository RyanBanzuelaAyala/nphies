...
using QInfrastructure.Api.Service.V3.PreAuthorization.Query;

namespace QApi.V3.Controller.Preauthorization
{
    [Produces("application/json")]
    [ApiVersion("3.0")]
    [AllowAnonymous]
    public class PreauthorizationController : QBaseController
    {
        private readonly IMapper _mapper;
        private readonly IMediator _mediator;
        private readonly IHttpEligibility _http;

        public PreauthorizationController(IMapper mapper, IMediator mediator, IHttpEligibility http)
        {
            _mapper = mapper;
            _mediator = mediator;
            _http = http;
        ...
        [HttpPost, Route("IsAuthorized")]
        public async Task<ActionResult> IsAuthorized([FromBody] NAPHIES_Eligibility_Param param, [FromHeader] string BranchId)
        {
            ...
            try
            {
                var resultMediator = await _mediator.Send(new PreAuthorizationQuery
                {
                    param = param,
                    BranchId = BranchId
                });
